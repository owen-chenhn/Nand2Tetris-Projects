...
int main(int argc, char** argv) {
    ...
    for (int i=out_path.length()-1; i >= 0 && count < 2; i--) {
        if (out_path[i] == '/') {
            count++;
            if (count == 1) 
                tail = i;
    ...
    if (head == 0) 
        out_path = out_path.substr(0, tail+1) + out_path.substr(0, tail) + ".asm";
    else
        out_path = out_path.substr(0, tail) + out_path.substr(head, tail-head) + ".asm";
    ...
    // Read in and process multiple vm files
    string in_path, class_name;
    list<string> text;
    for (int i = 1; i < argc; i++) {
        in_path = argv[i];
        class_name = in_path.substr(tail+1, in_path.length()-tail-4);
        cout << "Read in vm file: " << in_path << endl;
        // strip the comments and empty lines
        text_strip(text, in_path);
        // Decode the vm instructions and write to output file:
        cout << "Translate class file: " << class_name << endl;
